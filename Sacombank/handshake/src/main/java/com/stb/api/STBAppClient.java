...
public class STBAppClient {
		...
		(new Thread() {
			...
			public void run() {
				...
				try {
					...
					connection.setDoOutput(true);
					// Write stream
					System.out.println("data before send =  " + gson.toJson(businessObject));
					BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(connection.getOutputStream()));
					writer.write(gson.toJson(model));
					writer.close();
					// Read stream
					...
					model = gson.fromJson(reader.readLine(), STBModel.class);
					reader.close();
                    System.out.println("data after send =  " + gson.toJson(model));
                    ...
                    if(model!=null && model.RespCode!=null && model.RespCode.equals("00"))
	...
	public static String getSessionKey() {return _sessionKey;}
}
